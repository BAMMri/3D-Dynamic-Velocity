...
def main():
    ...
    for key, value in config.items():
        if key == 'CardiacNumberOfImages':
            # The 'Time' dimension is the 4th axis (index 3) of the velocities array
            info[key] = velocities.shape[3]
        elif key == 'NofPoints':
            # The 'X' dimension is the first axis (index 0) of the velocities array
            ...
            # Handle lists (like InPlaneResolution)
            info[key] = value
        else:
            # Handle scalar values
            info[key] = float(value) if isinstance(value, str) else value
