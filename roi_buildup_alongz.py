...
file_path1 = '/Volumes/ExtremeSSD/AA_PhD_Projects/MRI_Koper/subjects/01/Eig_763.npy' # First dataset path (required)
file_path2 = None # Second dataset path (optional)

# Set default values (to be overridden by config if available)
default_slice_idx = 23
default_dim4_idx = 0
repetition_time = 7.2  # ms
...
#     print("expanded mask", mask_bool.shape)
    ...
    print("Mask applied. Number of nonzero voxels:", np.count_nonzero(data1))
    # Expand mask if necessary to match data1's phases dimension
    if mask_bool.shape != data1.shape:
        if mask_bool.ndim == 3 and data1.ndim == 4:
            mask_bool = np.repeat(mask_bool[:, :, :, np.newaxis], data1.shape[3], axis=3)
        else:
            raise ValueError("Mask shape does not match data1 shape!")
...
img_plot.set_clim(vmin=0, vmax=10)
...
def update(val):
    ...
    img_plot.set_clim(vmin=vmin, vmax=vmax)
    # Update colorbar
